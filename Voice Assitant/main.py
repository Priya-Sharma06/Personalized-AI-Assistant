...
def wishMe():
    ...
    if (hour >= 0) and (hour <= 12) and ('AM' in t):
        speak(f"Good morning Priya, it's {day} and the time is {t}")
    elif (hour >= 12) and (hour <= 16) and ('PM' in t):
        speak(f"Good afternoon Priya, it's {day} and the time is {t}")
    else:
        speak(f"Good evening Priya, it's {day} and the time is {t}")
...
def openApp(command):
    if "calculator" in command:
        ...
        speak("Opening Paint")
        os.startfile('C:\\Users\\283pr\\AppData\\Local\\Microsoft\\WindowsApps\\mspaint.exe')
